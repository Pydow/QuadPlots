...
public class AddMemberCommand extends SubCommand {
    ...
    public AddMemberCommand() {
        super("addmember", "/p addmember <player>", new String[]{}, "addmember", false);
    }
                ...
                if (cb.plot.owner.equalsIgnoreCase(player.getName()) || isBypassing) {
                    if (args.length >= 2) {
                        ...
                        if (!cb.plot.members.contains(target)) {
                            // helper-added
                            QuadPlots.provider.addMember(cb.plot, target);
                            sender.sendMessage(Language.getAndReplace("member-added", target));
                        } else sender.sendMessage(Language.getAndReplace("already-member", target));
