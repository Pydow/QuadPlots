...
public class UndenyCommand extends SubCommand {
    ...
    public UndenyCommand() {
        super("undeny", "/p undeny <player>", new String[]{}, "deny", false);
    }
                ...
                if (cb.plot.owner.equalsIgnoreCase(player.getName()) || isBypassing) {
                    if (args.length >= 2) {
                        ...
                        if (cb.plot.denied.contains(target)) {
                            // helper-added
                            QuadPlots.provider.undenyPlayer(cb.plot, target);
                            sender.sendMessage(Language.getAndReplace("deny-removed", target));
                        } else sender.sendMessage(Language.getAndReplace("not-denied", target));
